...
const getTotalBalance = arr => {
    return arr.reduce((count, el) => count + el.balance, 0);
};
...
const getUsersByFriend = (arr, name) => {
    return arr.filter(el => el.friends.find(el => el === name)).map(el => el.name);
};
...
const getAllSkills = arr => {
    let res = arr.map(el => el.skills).reduce((a, b) => a.concat(b), []);
    return res.filter((a, b) => res.indexOf(a) === b).sort();
};
